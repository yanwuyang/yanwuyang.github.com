function fileQueueError(file, errorCode, message) {
	try {
		...
		addReadyFileInfo(file.id,file.name,file.type,file.size,imageName);
...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,file.type,file.size,"加入队列");
}
...
function uploadProgress(file, bytesLoaded) {
	try {
		...
		row.cells[3].innerHTML = getTplStr(percent);
		row.cells[4].innerHTML = "正在上传";
	} catch (ex) {
		this.debug(ex);
	}
}
...
function getTplStr(v){
	return '<div>'+
				'<div style="border:1px solid #7491B3;height:12px;width:90px;margin:2px 0px 1px 0px;float:left;">'+		
					'<div style="float:left;background:#6D9FDE;width:'+v+'%;height:12px;"><div></div></div>'+
				'</div>'+
...
function addFileInfo(fileId,message){
	...
	row.cells[4].innerHTML = "<font color='green'>"+message+"</font>";
}
function addReadyFileInfo(fileid,fileName,fileType,fileSize,status){
	//用表格显示
	...
	col3.innerHTML = (fileSize/1024).toFixed(2);
	col5.innerHTML = status;
	col6.innerHTML = "<img src='../../../styles/base/image/delete.png' onclick='deleteFile(\""+fileid+"\")' title='删除' style='cursor:pointer'/>";
	col5.align="center";
	col6.align="center";
}
