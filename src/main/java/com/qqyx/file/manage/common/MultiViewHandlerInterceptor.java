...
public class MultiViewHandlerInterceptor extends HandlerInterceptorAdapter implements LogEnabled{
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(null!=session.getAttribute("resourceItems")){
		request.setAttribute("resourceItems", session.getAttribute("resourceItems"));
		session.removeAttribute("resourceItems");
		}
		
		BaseController resource=null;
		
		if(handler instanceof BaseController){
			 resource=(BaseController)handler; 
		}
		
		if(handler instanceof HandlerMethod){
			HandlerMethod handlerMethod=(HandlerMethod)handler;
			if(handlerMethod.getBean() instanceof BaseController){
				resource=(BaseController)handlerMethod.getBean();
			}
		}
		if(resource!=null){
	...
	private JSONObject getParameter(HttpServletRequest request) throws JSONException {
		...
		if(!paramap.isEmpty()){
			...
			for(String key:paramap.keySet()){
				if(JSONArray.fromObject(paramap.get(key)).size()>1) {
